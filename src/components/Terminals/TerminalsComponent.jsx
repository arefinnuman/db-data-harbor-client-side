...
export default function TerminalsComponent() {
  ...
  const { data, isLoading, refetch } = useGetAllTerminalsQuery(undefined, {
  ...
  const columns = [
    { name: "Name", selector: (row) => row.terminalNameAndId, sortable: true },
    { name: "Id", selector: (row) => row.terminalId, sortable: true },
    { name: "Type", selector: (row) => row.terminalType, sortable: true },
    {
      name: "Status",
      selector: (row) => row.terminalStatus,
      sortable: true,
    },
    {
      name: "Brand Name",
      selector: (row) => row.terminalBrand,
      sortable: true,
    },
    {
    ...
    },
    {
      name: "Live Date",
      selector: (row) =>
        row.liveDate ? new Date(row.liveDate).toLocaleDateString() : "-",
      sortable: true,
    },
    {
      name: "Details",
      ...
      ? [
          {
            ...
            cell: (row) => (
              <button
                onClick={() => setSelectedUpdateTerminal(row)}
  ...
  //   custodiansKey: { name: 'Carol', contactNumber: '555-333-2222' },
  //   custodiansCom: { name: 'Smith', contactNumber: '555-333-2222' },
  ...
  const openModal = () => {
    ...
    const selectedDataToExport = filteredData
      ...
      .map((row) => ({
        "Terminal Name": row.terminalNameAndId,
        "Terminal Id": row.terminalId,
        "Terminal Type": row.terminalType,
        "Terminal Status": row.terminalStatus,
        "Terminal Brand": row.terminalBrand,
        "Terminal Model": row.terminalModel,
        "GL Number": row.glNumber,
        "GL Code": row.glCode,
        "Number of BPM": row.numberOfBpm,
        "Insurance Limit": row.insuranceLimit,
        "Asset Tag Serial": row.assetTagSerial,
        "Deployment Date": row.deploymentDate
          ? new Date(row.deploymentDate).toLocaleDateString()
          : "-",
        "Live Date": row.liveDate
          ? new Date(row.liveDate).toLocaleDateString()
          : "-",
        "Monthly No of Transaction": row.monthlyNoOfTransaction,
        "Monthly Vol of Transaction": row.monthlyVolOfTransaction,
        "Monthly Avg No of Txn": row.monthlyAvgNoOfTxn,
        "Monthly Avg Vol of Txn": row.monthlyAvgVolOfTxn,
        "EBL365 Name": row.ebl365?.ebl365Name,
        "EBL365 Address": row.ebl365?.ebl365Address,
        "EBL365 Zone": row.ebl365?.ebl365Zone,
        "EBL365 Status Type": row.ebl365?.ebl365StatusType,
        "Location Type": row.ebl365?.locationType,
        "Area Type": row.ebl365?.areaType,
        "Booth Devices": row.ebl365?.boothDevices,
      }));
      ...
      {isLoading ? (
        ...
        <section className="md:px-5">
          ...
          {selectedUpdateTerminal && (
            <Modal
              isOpen={true}
              onRequestClose={() => setSelectedUpdateTerminal(null)}
